...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.crypto_currency_exchange?.length > 0) {
      const create_crypto_currency_exchange = body.crypto_currency_exchange;
      body.crypto_currency_exchange = {
        create: create_crypto_currency_exchange,
      };
    } else {
      delete body.crypto_currency_exchange;
    }
